...
# We make our linear model
model <- lm(mpg ~ wt, data=mtcars);model
summary(model)

b1Est = model$coefficients[2];b1Est
b0Est = model$coefficients[1];b0Est
bHat = rbind(b0Est,b1Est);bHat
Y = mtcars$mpg;Y
X = mtcars$wt;X
YHat = b0Est + b1Est*X;YHat

beetaHat = (Y-epsilon)/X;beetaHat[,1]


# Two ways of getting the epsilon of the model:
epsilon = resid(model);epsilon
epsilon2 = Y - YHat;epsilon2
```

```{r}
?arrows()

arrows(X,Y,X=X,YHat=Y)
```
...
```{r, eval=FALSE}
# Since the real Y-values (Y-values, not Y-hat (the estimated Y value)) are defined by the fuel usage, we can simply extract the Y-values by taking them from the column of interest in the mtcars dataset
Y = mtcars$mpg  %>% 
  t();Y
# We transpose the column for later use
...
The estimated slope of the modeled linear relationship between the two variables, weight and fuel usage, or the $\hat{\beta}$ is therefore: -5.344472

```{r, eval=FALSE}
?data()

const <- rep(1,length(mtcars$mpg))
...
modelMatrix = model.matrix(model);modelMatrix

modelMatrix

mtcars["wt"];modelMatrix
