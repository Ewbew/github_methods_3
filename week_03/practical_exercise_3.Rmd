...
# We now make the model, specifying the family to be poisson, based on the nature of the data's distribution (no intercept for subject is being modeled - only the slope for the variable for each subject):
count_model4 = glmer(count ~ pas*task + (pas+0|subject),data = dataCount, family = poisson) 
summary(count_model4)
count_model2 = glmer(count ~ pas*task + (pas|subject), data = dataCount, family = poisson)
