...
pacman::p_load(utils,tidyverse, patchwork, ggplot2, lme4, stats, grid, ggpubr, ggrepel, graphics,effects,VCA, vroom, readbulk,stringi, gridExtra)
```
...
plot <- ggplot(plot_dat)+
  geom_point(aes(x = target.contrast, y = fitted), color = "pink")+
  geom_line(aes(x = target.contrast, y = fitted), color = "pink")+
  geom_point(aes(x = target.contrast, y = yEstPartialPooling), color = "orange")+
  geom_line(aes(x = target.contrast, y = yEstPartialPooling), color = "orange")+
  xlab('Target Contrast')+
  ylim(c(0,1))+
  theme_minimal()
return(plot)
...
poolNoPoolFun <- function(i){
  ...
  plot_dat <- data.frame(cbind(fitted,'target.contrast'=dat$target.contrast,'yEstPartialPooling' = dat$yEstPartialPooling))
plot <- ggplot(plot_dat)+
  geom_point(aes(x = target.contrast, y = yEstPartialPooling))+
  geom_line(aes(x = target.contrast, y = yEstPartialPooling))+
  xlab('Target Contrast')+
  ylim(c(0,1))+
  theme_minimal()
 return(plot)
