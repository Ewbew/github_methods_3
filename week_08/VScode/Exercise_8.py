...
# find random training indices
from random import sample
np.random.seed(7)
indices = np.arange(0, len(y_equal))

train_indices = sample(range(len(y_equal)), 120) test_indices = np.setdiff1d(indices, train_indices)
...
# 3.1.3 Run the sample-by-sample analysis (similar to Exercise 2.2.v) with the best kernel (from Exercise 3.1.ii). Make a plot with time on the x-axis and classification score on the y-axis with a horizontal line at the chance level (what is the chance level for this analysis?)
...
# 3.2.1: Use the kernel that resulted in the best classification in Exercise 3.1.ii and fitthe training set and predict on the test set. This time your features are the number of sensors multiplied by the number of samples.
